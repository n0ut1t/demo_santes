...
const SONGS = [
  {
    ...
    audioSrc: 'Bequetero.mp3',
    videoSrc: '../../../audio/Bequetero.mp3',
    bgColor: '#1a0a2e',
    bgGradient: 'linear-gradient(135deg,#1a0a2e,#0d1a0a)',
    bgImage: 'https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?w=800&q=80',
    duration: 119.81,
  ...
  },
  {
    ...
    artist: 'Les Santes de Mataró',
    audioSrc: null,
    bgImage: 'https://images.unsplash.com/photo-1574184864703-3487b13f0edd?w=800&q=80',
    duration: 90,
    locked: true
  },
  {
...
const LANE_COLORS = ['#e8002d','#0099ff','#00cc55','#ff8800'];
const LANE_COLORS_A = ['rgba(232,0,45,0.18)','rgba(0,153,255,0.18)','rgba(0,204,85,0.18)','rgba(255,136,0,0.18)'];
...
(function initParticles() {
  ...
  function frame() {
    ...
    for(const p of pts) {
      ...
      ctx.fillStyle = `rgba(200,168,75,${p.a*0.5})`;
      ctx.fill();
    }
...
function buildSongGrid() {
  ...
  for(const song of SONGS) {
    ...
    info.innerHTML = `<div class="song-card-title">${song.title}</div><div class="song-card-meta">${song.artist} · ${Math.floor(song.duration/60)}:${String(Math.floor(song.duration%60)).padStart(2,'0')}</div>`;
...
function startSong(song, exMode = false) {
  ...
  G = {
    song,
    isEx,
    notes: (isEx ? song.notesHard : song.notesEasy).map((n,i) => ({
      time: isEx ? n[0]*0.5 : n[0],  // 2x = half the time values (faster)
      lane: n[1],
      hit: false, missed: false, _y: -999, idx: i
    })),
    noteIdx: 0,
    activeNotes: [],
    score: 0,
  ...
  aud.play().then(() => {
    ...
    // no audio — run on timer
    const t0 = performance.now();
    G.audio = { get currentTime(){ return (performance.now()-t0)/1000 * (isEx?2:1); }, pause:()=>{} };
    G.running = true;
    G.animId = requestAnimationFrame(gameLoop);
  });
}
...
function getSongTime() {
  return G.audio ? G.audio.currentTime : 0;
}
 
const SPEED = 340; // px/sec — faster tiles
function getHitY() { return canvas.height - 100; }
const HIT_ZONE_HALF = 55; // px above/below hit line counts as valid hit
 
function spawnNotes(t) {
  const hitY = getHitY();
  while(G.noteIdx < G.notes.length) {
    const n = G.notes[G.noteIdx];
    const spawnT = n.time - hitY / SPEED;
    if(spawnT <= t) { G.activeNotes.push(n); G.noteIdx++; }
    else break;
  }
}
...
function drawGame(t) {
  ...
  // Hit zone visual — big clear area so players know where to tap
  for(let i=0;i<4;i++) {
  ...
  // Notes — compute Y from time difference to song position
  for(const n of G.activeNotes) {
    if(n.hit||n.missed) continue;
    // Y = hitY + (note_time - current_time) * SPEED  → note above hit line when time > 0
    const y = hitY - (n.time - t) * SPEED - NOTE_H/2;
    n._y = y + NOTE_H/2; // center Y for hit detection
 
    const x = n.lane * LANE_W;
    const nW = LANE_W - 10, nH = NOTE_H;
    const nx = x+5, ny = y;
 
    ctx2.shadowColor = LANE_COLORS[n.lane];
    ctx2.shadowBlur = 16;
    ctx2.fillStyle = LANE_COLORS[n.lane];
    roundRect(ctx2, nx, ny, nW, nH, 7);
    ctx2.fill();
    ctx2.shadowBlur = 0;
 
    // Shine strip
    ctx2.fillStyle = 'rgba(255,255,255,0.22)';
    roundRect(ctx2, nx+2, ny+2, nW-4, nH*0.38, 5);
    ctx2.fill();
  }
...
function gameLoop(ts) {
  ...
  if(t >= dur && G.noteIdx >= G.notes.length && G.activeNotes.every(n=>n.hit||n.missed)) { endGame(); return; }
  G.animId = requestAnimationFrame(gameLoop);
}
...
function tapLane(lane, e) {
  ...
  // Find closest note in this lane that's within the visual hit zone
  let best = null, bestDist = Infinity;
  for(const n of G.activeNotes) {
    if(n.hit||n.missed||n.lane!==lane) continue;
    // Distance from center of note to hit line, in pixels
  ...
  if(best && bestDist <= HIT_ZONE_HALF + 20) {
    ...
    const col = pts===300?'#ffdd00':pts===200?'#00ccff':'#ffffff';
    showFeedback(lane, label, col);
    flashLane(lane,'hit');
    updateHUD();
  } else {
    // Only penalize if there's actually a note nearby that was missed
    flashLane(lane,'miss');
  }
}
...
function showFeedback(lane, text, color) {
  ...
  el.style.left = (lane*lW + lW/2 - 30)+'px';
...
function updateHUD() {
  ...
  c.textContent = G.combo > 1 ? 'x'+G.combo : '';
...
function endGame() {
  ...
  if(G.audio && G.audio.pause) G.audio.pause();
 
  const total = G.totalNotes;
  const hitPct = G.hitCount / total;
  const stars = hitPct >= 0.95 ? 3 : hitPct >= 0.75 ? 2 : hitPct >= 0.5 ? 1 : 0;
 
  const sd = getSongData(G.song.id);
  const isNewBest = G.score > sd.bestScore;
  ...
  for(let i=1;i<=3;i++) {
    const s = document.createElement('span');
    s.style.color = stars>=i ? '#c8a84b':'#333';
    s.textContent = '★';
  ...
  // Retry button changes if ex available
...
document.addEventListener('keydown', e => {
  if(keysHeld.has(e.key)) return; // no repeat
  keysHeld.add(e.key);
